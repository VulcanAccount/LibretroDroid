...
// TODO... Do we really need this?
    ...
    void LibretroDroid::changeDisk(unsigned int index) {
        ...
        if (index < 0 ||
            index >= Environment::getInstance().getRetroDiskControlCallback()->get_num_images()) {
        ...
        if (Environment::getInstance().getRetroDiskControlCallback()->get_image_index() != index) {
            ...
            Environment::getInstance().getRetroDiskControlCallback()->set_image_index(
                    (unsigned) index);
    ...
    JNICALL LibretroDroid::unserializeSRAM(int8_t *data, size_t size) {
    ...
    float LibretroDroid::findDefaultAspectRatio(const retro_system_av_info &system_av_info) {
        ...
        if (result < 0) {
            result =
                    (float) system_av_info.geometry.base_width /
